...
```mermaid
graph TD
    A[Conselho de Governo<br/>Órgão Superior] --> B[CONAMA<br/>Órgão Consultivo e Deliberativo]
    A --> C[Ministério do Meio Ambiente<br/>Órgão Central]
    C --> D[IBAMA<br/>Órgão Executor Federal]
    C --> E[ICMBio<br/>Órgão Executor Federal]
    C --> F[Órgãos Seccionais<br/>Estados]
    F --> G[Órgãos Locais<br/>Municípios]
    B -.assessora.-> C
    D -.coordenação.-> C
    E -.coordenação.-> C
```
...
```mermaid
graph LR
    A[Requerimento<br/>da Licença] --> B[Elaboração de<br/>Estudos Ambientais]
    B --> C[Análise pelo<br/>Órgão Ambiental]
    C --> D{Audiência Pública<br/>se cabível}
    D --> E[Emissão da<br/>Licença Prévia - LP]
    E --> F[Detalhamento<br/>do Projeto]
    F --> G[Emissão da<br/>Licença de Instalação - LI]
    G --> H[Construção do<br/>Empreendimento]
    H --> I[Emissão da<br/>Licença de Operação - LO]
    I --> J[Operação e<br/>Monitoramento]
```
