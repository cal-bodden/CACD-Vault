...
```mermaid
graph TD
    A[Paisagem Natural Brasileira] --> B(Domínios Morfoclimáticos)
    A --> C(Biomas)
    B --> D[Núcleo do Domínio]
    B --> E[Faixas de Transição]
    C --> F[Tipos de Vegetação]
    F -- compõe --> B
    F -- compõe --> C
    G[Ecossistemas] -- podem estar contidos em --> B
    G -- podem estar contidos em --> C
```

> [!example] Componentes de um Domínio Morfoclimático (Mermaid - Domínio dos Cerrados)
```mermaid
graph TD
    H[Domínio dos Cerrados]
    H --> I(Clima Tropical Sazonal)
    H --> J(Relevo de Chapadões e Planaltos)
    H --> K(Solos Latossólicos Ácidos)
    H --> L(Vegetação Savânica Diversificada)
    H --> M(Rede Hidrográfica Importante - Berço das Águas)
    I <--> L
    J <--> L
    K <--> L
    M <--> L
