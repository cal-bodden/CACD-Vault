...
```mermaid
graph TD
    subgraph Estado-Membro
        A[Governo envia relatorio periodico sobre convencao ratificada]
        B[Organizacoes de empregadores e trabalhadores podem enviar informacoes]
    end
    
    subgraph OIT-Nivel-Tecnico [OIT - Nivel Tecnico]
        C[Comite de Peritos - CEACR]
        C_Desc[20 juristas independentes - Analise tecnica e imparcial]
    end
    
    subgraph OIT-Nivel-Politico [OIT - Nivel Politico]
        E[Comissao de Aplicacao de Normas - CAN]
        E_Desc[Orgao tripartite da Conferencia Internacional do Trabalho]
    end
    
    F[Publicacao das conclusoes no relatorio da Conferencia - Pressao politica e diplomatica]
    
    A --> C
    B --> C
    C_Desc --> C
    C --> |Relatorio Anual com Observacoes| E
    E_Desc --> E
    E --> |Seleciona Short List de casos para discussao| E
    E --> |Dialogo com Governo, Empregadores e Trabalhadores| F
```
