...
```mermaid
flowchart TD
    A[Consultas bilaterais - ate 60 dias] --> B[Estabelecimento de Painel - por decisao do OSC]
    B --> C[Exame do Painel e Relatorio final - 6-9 meses]
    C --> D{Apelacao ao Orgao de Apelacao?}
    D -- Sim --> E[Relatorio do Orgao de Apelacao - ate 90 dias]
    D -- Nao --> F[Relatorio do Painel segue para adocao]
    E --> F[Relatorio final para adocao pelo OSC]
    F --> G[Implementacao das recomendacoes pelo pais vencido]
    G --> H{Cumprimento efetivo?}
    H -- Sim --> I[Encerramento da controversia]
    H -- Nao --> J[Arbitragem sobre retaliacao - Art.22.6]
    J --> K[Autorizacao de retaliacao - suspensao de concessoes]
    K --> L[Retaliacao em vigor - ate solucao ou compliance]
```
