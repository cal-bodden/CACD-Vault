...
flowchart TB
  ...
  subgraph UE["União Europeia – Modelo Supranacional"]
    direction TB
    A["Instituições da UE aprovam norma (ex: Regulamento ou Diretiva)"]
    B{"Tipo de Ato"}
    C["Aplicação direta em cada Estado-membro"]
    D["Transposição necessária pelo Estado-membro (lei nacional)"]
  ...
  subgraph Mercosul["Mercosul – Modelo Intergovernamental"]
    direction TB
    E["Órgãos do Mercosul adotam norma (Decisão/Resolução)"]
    F["Incorporação interna obrigatória em cada Estado"]
    G["Norma implementada via decreto ou lei nacional"]

    E --> F --> G
  end

  %% Estilo dos blocos
  style UE fill:#e0f7fa,stroke:#333,stroke-width:1px,color:#000
  style Mercosul fill:#ffe0b2,stroke:#333,stroke-width:1px,color:#000
