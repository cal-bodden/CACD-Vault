...
flowchart TD
    %% ========= Carta =========
    Charter["Carta das Nações Unidas"]:::charter

    %% ========= ÓRGÃOS PRINCIPAIS =========
    subgraph "Estrutura Central – Órgãos Principais"
        direction TB
        GA["Assembleia Geral"]:::principal
        SC["Conselho de Segurança"]:::principal
        ECOSOC["ECOSOC"]:::principal
        ICJ["Corte Internacional<br>de Justiça"]:::principal
        Secretariat["Secretariado (SG)"]:::principal
        TC["Conselho de Tutela<br>(‑suspenso‑)"]:::principal
    end
    ...
    %% ========= FAMÍLIA ONU =========
    subgraph "Família ONU"
        direction TB
        %% Fundos‑Programas
        subgraph "Fundos & Programas  ↦  órgãos subsidiários da AG"
            UNICEF["UNICEF – Infância"]:::fundos
            WFP["PMA/WFP – Alimentação Humanitária"]:::fundos
            UNDP["PNUD/UNDP – Desenvolvimento"]:::fundos
            UNHCR["ACNUR/UNHCR – Refugiados"]:::fundos
        end

        %% Agências
        subgraph "Agências Especializadas  ↦  organizações autônomas"
            WHO["OMS/WHO – Saúde"]:::agencia
            FAO["FAO – Alimentação & Agricultura"]:::agencia
            UNESCO["UNESCO – Educação, Ciência & Cultura"]:::agencia
            ILO["OIT/ILO – Trabalho"]:::agencia
            IMF["FMI/IMF – Finanças"]:::agencia
            WB["Banco Mundial"]:::agencia
        end
    end
    ...
    GA -- "elege" --> ECOSOC
    GA -- "elege juízes" --> ICJ
    GA -- "elege 10 membros<br>não permanentes" --> SC
    SC -- "recomenda SG" --> Secretariat

    %% Fundos & Programas
    GA -- "cria / supervisiona" --> UNICEF
    GA -- "cria / supervisiona" --> WFP
    GA -- "cria / supervisiona" --> UNDP
    GA -- "cria / supervisiona" --> UNHCR

    ECOSOC -- "coordena / recebe relatórios" --> UNICEF
    ECOSOC --> WFP
    ECOSOC --> UNDP
    ECOSOC --> UNHCR

    %% Agências Especializadas (Art. 63 da Carta)
    ECOSOC -- "acordos de relação" --> WHO
    ECOSOC --> FAO
    ECOSOC --> UNESCO
    ECOSOC --> ILO
    ECOSOC --> IMF
    ...
    %% ========= ESTILOS =========
    classDef charter   fill:#fff2cc,stroke:#333,stroke-width:2px;
    classDef principal fill:#dae8fc,stroke:#333,stroke-width:1px;
    classDef fundos    fill:#d5e8d4,stroke:#497a3a,stroke-width:1px;
    classDef agencia   fill:#ffe6cc,stroke:#b36a00,stroke-width:1px;

    class Charter charter;
    class GA,SC,ECOSOC,ICJ,Secretariat,TC principal;
    class UNICEF,WFP,UNDP,UNHCR fundos;
    class WHO,FAO,UNESCO,ILO,IMF,WB agencia;
```
