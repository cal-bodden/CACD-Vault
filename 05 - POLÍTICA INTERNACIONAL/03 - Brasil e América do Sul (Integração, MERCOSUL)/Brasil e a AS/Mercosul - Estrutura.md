...
flowchart TB
    ...
    direction TB
        ...
        CCM["Comissão de Comércio do Mercosul (CCM)<br>Órgão técnico - política comercial"]
        CMC --> GMC --> CCM
    end
    Parlasul["Parlamento do Mercosul (Parlasul)<br>Órgão representativo consultivo"]
    FCES["Foro Consultivo Econômico-Social (FCES)<br>Órgão consultivo setorial"]
    SAM["Secretaria do Mercosul (SAM)<br>Apoio administrativo - Montevidéu"]
    TPR["Tribunal Permanente de Revisão (TPR)<br>Órgão judicial arbitral"]
...
%% Aplicando as cores
classDef decisorio fill:#0074D9,color:#ffffff,stroke:#003366,stroke-width:1px;
classDef consultivo fill:#2ECC40,color:#ffffff,stroke:#186A1D,stroke-width:1px;
classDef jurisdicional fill:#FF4136,color:#ffffff,stroke:#990000,stroke-width:1px;

class CMC,GMC,CCM decisorio;
class Parlasul,FCES,SAM consultivo;
class TPR jurisdicional;
...
```mermaid
flowchart TB
    A["Controvérsia entre Estados Partes"] --> B["Negociações diretas<br>(até 15 dias)"]
    B -->|"Não há acordo"| C{"Submeter à mediação do GMC?"}
    C -->|Sim| D["GMC analisa e recomenda solução<br>(até 30 dias)"]
    C -->|"Não (ou sem solução)"| E["Solicitação de Arbitragem Ad Hoc"]
    D --> E
    E --> F["Tribunal Arbitral Ad Hoc (3 árbitros)<br>Julga a controvérsia (60 + 30 dias)"]
    F --> G["Laudo Arbitral"]
    G --> H{"Recurso de revisão ao TPR?"}
    H -->|"Sim, em até 15 dias"| I["Tribunal Permanente de Revisão (TPR)<br>Julga recurso (30 + 15 dias)"]
    H -->|Não| J["Laudo Ad Hoc torna-se definitivo"]
    I --> K["Laudo do TPR (definitivo e vinculante)"]
    J --> K
    K --> L["Estado Parte vencido deve cumprir o laudo<br>(prazo estipulado ou 30 dias)"]
    L --> M{"Cumpriu integralmente?"}
    M -->|Sim| N["Fim da controvérsia"]
    M -->|Não| O["Adoção de medidas compensatórias temporárias<br>pelo Estado vencedor (ex.: suspensão de concessões comerciais)"]
    O --> P["Possibilidade de contestar medidas compensatórias<br>perante novo tribunal (30 dias para decisão)"]
    P --> Q["Laudo sobre medidas<br>(ajuste ou confirmação das sanções)"]
    Q --> R["Fim: cumprimento do laudo e/ou retirada das sanções"]
