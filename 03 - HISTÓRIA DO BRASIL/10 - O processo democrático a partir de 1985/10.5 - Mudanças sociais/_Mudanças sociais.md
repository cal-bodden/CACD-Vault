...
```mermaid
graph TD
    subgraph Pirâmide_Etária_do_Brasil_1985_Estimativa
        A1["A1"]
        B1["B1"]
        C1["Formato: Pirâmide Clássica"]
        A1 --> B1
        B1 --> C1
    end

    subgraph Pirâmide_Etária_do_Brasil_2025_Projeção
        A2["A2"]
        B2["B2"]
        C2["C2"]
        A2 --> B2
    ...
    style node1985 fill:#f9f,stroke:#333,stroke-width:2px
    style node2025 fill:#ccf,stroke:#333,stroke-width:2px
...
```mermaid
graph TD
    subgraph "Trilha da Inclusão Social (Elevação do Piso)"
        A --> B{Políticas Sociais Ativas};
        B --> C1;
        B --> C2;
        C1 --> D;
        C2 --> D;
    ...
    subgraph "Trilha da Desigualdade Estrutural (Manutenção do Teto)"
        F["Estrutura Histórica<br><i>(Herança Colonial e Escravocrata)</i>"] --> G{Mecanismos de Perpetuação};
        G --> H1;
        G --> H2;
        H1 --> I;
        H2 --> I;
