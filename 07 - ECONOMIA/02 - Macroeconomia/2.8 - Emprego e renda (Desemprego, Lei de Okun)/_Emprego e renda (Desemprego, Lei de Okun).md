...
flowchart LR
    subgraph Visão Clássica
      direction TB
      X1(Salários flexíveis)
      X2(Equilíbrio de pleno emprego)
      X3(Desemprego apenas voluntário/natural)
      X1 --> X2 --> X3
    end
    subgraph Visão Keynesiana
      direction TB
      Y1(Rigidez de salários/preços)
      Y2(Demanda agregada determina o emprego)
      Y3(Desemprego involuntário possível)
      Y1 --> Y2 --> Y3
    end
```
