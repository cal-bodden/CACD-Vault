...
```mermaid
flowchart LR
    BC[Banco Central opera no mercado aberto]
    BC -->|"Compra títulos (injeta moeda)"| LIQp[Aumenta liquidez no sistema]
    BC -->|"Venda títulos (retira moeda)"| LIQm[Reduz liquidez no sistema]
    LIQp -->|"Queda da taxa Selic"| rateDown[Queda da taxa Selic]
    LIQm -->|"Alta da taxa Selic"| rateUp[Alta da taxa Selic]
    rateDown -->|"Mais crédito, consumo e investimento"| creditUp[Mais crédito, consumo e investimento]
    rateUp -->|"Menos crédito, consumo e investimento"| creditDown[Menos crédito, consumo e investimento]
    creditUp -->|"Possível pressão inflacionária"| infUp[Possível pressão inflacionária]
    creditDown -->|"Alívio em pressões inflacionárias"| infDown[Alívio em pressões inflacionárias]
