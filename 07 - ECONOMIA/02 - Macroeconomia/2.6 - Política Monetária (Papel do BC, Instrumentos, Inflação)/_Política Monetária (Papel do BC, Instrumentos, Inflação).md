...
```mermaid
%% Gráfico simplificado da balança comercial no tempo após desvalorização
graph TD
    A(Início da desvalorização)
    B(Piora da balança comercial no curto prazo)
    C(Recuperação da balança comercial no médio prazo)
    D(Melhora da balança comercial no longo prazo)
    
    A --> B --> C --> D
    style B fill:#f9d5d3,stroke:#f44336,stroke-width:2px
    style C fill:#d1f0d1,stroke:#4caf50,stroke-width:2px
    style D fill:#a3d977,stroke:#388e3c,stroke-width:2px
```
